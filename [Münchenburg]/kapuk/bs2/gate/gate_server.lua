...
local garage1 = createObject(968, -2050.3000488281,102.2, 35, 0, 90, 0)
...
function hasznalasgate(thePlayer)
	...
	local distance1 = getDistanceBetweenPoints3D(-2050.3000488281,102.2, 35, x,y,z)
	local distance2 = getDistanceBetweenPoints3D(-2050.3000488281,102.2, 35, x, y, z)

	if (distance1<=10) and (open==false) then
			open = true
			which = 1
			outputChatBox("#FF0000 A kapu #FFFF00 nyitva!", thePlayer, 0, 0, 0, true)
			moveObject(garage1, 1000, -2050.3000488281,102.2, 35, 0, 0, 0)
...
function bezarasgate(thePlayer)
	...
	if (which==1) then
		which = 0
		moveObject(garage1, 1000, -2050.3000488281,102.2, 35, 0, 90, 0)
--	elseif (which==2) then
--		which = 0
