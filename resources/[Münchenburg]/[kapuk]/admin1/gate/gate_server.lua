local garage1 = createObject(8378, 96.599609375,-1274.7001953125, 14.39999961853, 0, 0, 309.99572753906)
...
function hasznalasgate(thePlayer)
	...
	local distance1 = getDistanceBetweenPoints3D(96.599609375,-1274.7001953125, 14.39999961853, x,y,z)
	local distance2 = getDistanceBetweenPoints3D(96.599609375,-1274.7001953125, 14.39999961853, x,y,z)

	if (distance1<=30) and (open==false) then
			open = true
			which = 1
			outputChatBox("#FF0000 A kapu #FFFF00 nyitva!", thePlayer, 0, 0, 0, true)
			moveObject(garage1, 1000, 250,-1274.7001953125, 14.39999961853, 0, 0,0)
...
function bezarasgate(thePlayer)
	...
	if (which==1) then
		which = 0
		moveObject(garage1, 1000, 96.599609375,-1274.7001953125, 14.39999961853, 0, 0, 0)
--	elseif (which==2) then
--		which = 0
